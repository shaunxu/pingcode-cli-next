...
3. merge 到 main 后 `.github/workflows/release-tag.yml` 先跑 `./scripts/live-test.sh` 全量 live 旅程作为门禁，通过后才创建并推送 annotated tag `vX.Y.Z`，tag 推送触发 cargo-dist 构建分发。门禁顺序：仅 release 提交（消息含 `chore(release): v`）才跑测试——普通 feat/fix PR 合并不跑测试也不打 tag；live 测试失败则不打 tag、不产生 GitHub Release；tag 已存在则幂等成功。失败可在 Actions 页面 re-run，或用 Actions 的 **Run workflow**（`workflow_dispatch`，可勾选 `skip-tests`）手动补发。**该 workflow 需要仓库 secret `RELEASE_PAT`**（fine-grained PAT，仅授权本仓库、Contents: Read and write）——用 Actions 默认 `GITHUB_TOKEN` 推的 tag 不会触发其他 workflow。
