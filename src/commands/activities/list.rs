...
/// `pc activities list` 的参数。
#[derive(Debug, Args)]
pub struct ListArgs {
    /// Type of the principal the activity records belong to
...
/// 获取活动记录列表：`GET /v1/activities`（分页，scope 依赖活动记录所属主体，
/// 如 workitem 需要 `pcp:read:pjm:workitem`）。
///
/// 查询参数：
/// - `principal_type`：主体类型（`workitem`/`testrun`/`testcase`/`idea`/
///   `ticket`）；
/// - `principal_id`：主体 id；
...
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getActivitiesByPrincipalTypeAndPrincipalId
pub async fn run(ctx: &Ctx, args: &ListArgs) -> anyhow::Result<()> {
    ...
    let response: Value = ctx
        .client
        .get_with_query("/v1/activities", &Value::Object(query))
        .await?;
