...
/// 获取项目下的工作项类型列表：`GET /v1/pjm/workitem/workitem_types`（分页，
/// scope: `pcp:read:pjm:workitem`）。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getPjmWorkitemWorkitemTypesByProjectId
pub async fn run(ctx: &Ctx, args: &ListForProjectArgs) -> anyhow::Result<()> {
    ...
    let response: Value = ctx
        .client
        .get_with_query("/v1/pjm/workitem/workitem_types", &Value::Object(query))
        .await?;
