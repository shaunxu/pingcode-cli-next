...
/// 创建一个看板栏：`POST /v1/pjm/projects/{project_id}/boards/{board_id}/entries`
/// （scope: `pcp:write:pjm:board`）。
///
/// 请求体必填 `name`（同一看板下唯一），可选 `wip_limit`（在制品数量）、
/// `is_split`（是否拆分为进行中和已完成，默认 false）、
/// `definition_of_done`（完成的定义）。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/postPjmProjectsByProjectIdBoardsByBoardIdEntries
