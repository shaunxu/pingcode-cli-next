use clap::Args;

use crate::commands::Ctx;

/// `pc pjm project get` 的参数。
#[derive(Debug, Args)]
pub struct GetArgs;

/// 获取一个项目：`GET /v1/pjm/projects/{project_id}`。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getPjmProjectsByProjectId
pub async fn run(_ctx: &Ctx, _args: &GetArgs) -> anyhow::Result<()> {
    todo!("GET /v1/pjm/projects/<project_id> — docs: https://developer.alpha.pingcode.live/restapi/pingcode/getPjmProjectsByProjectId")
}
