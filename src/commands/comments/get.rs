...
/// 获取一条评论：`GET /v1/comments/{comment_id}`（scope 依赖评论所属主体，
...
/// `principal_id`（评论主体 id）必填。被删除的评论 `content` 字段会被置空、
/// `is_deleted` 为 1。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getCommentsByCommentIdByPrincipalTypeAndPrincipalId
