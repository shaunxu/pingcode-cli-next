...
/// 删除一个评审：`DELETE /v1/reviews/{review_id}`（scope 依赖评审所属主体）。
///
/// 查询参数 `principal_type`（评审主体类型：`workitem`/`testcase`/`idea`）
/// 必填。返回被删除的评审对象。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/deleteReviewsByReviewIdByPrincipalType
