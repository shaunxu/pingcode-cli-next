...
/// `pc reviews remove-principal` 的参数。
#[derive(Debug, Args)]
pub struct RemovePrincipalArgs {
    /// Review id
...
/// 从评审中移除一个评审内容：
/// `DELETE /v1/reviews/{review_id}/principals/{principal_id}`
/// （scope 依赖评审所属主体）。
///
/// 查询参数 `principal_type`（评审主体类型：`workitem`/`testcase`/`idea`）
/// 必填。返回被移除的评审内容对象。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/deleteReviewsByReviewIdPrincipalsByPrincipalIdByPrincipalType
pub async fn run(ctx: &Ctx, args: &RemovePrincipalArgs) -> anyhow::Result<()> {
