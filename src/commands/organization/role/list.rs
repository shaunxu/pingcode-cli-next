...
/// 分页获取角色列表：`GET /v1/directory/roles`
/// （scope: `pcp:read:global:team`）。
///
/// 返回企业角色列表，含系统内置与自定义角色。
/// 响应为分页结构（`page_index` / `page_size` / `total` / `values`）。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getDirectoryRoles
pub async fn run(ctx: &Ctx, args: &ListArgs) -> anyhow::Result<()> {
    ...
    let response: Value = ctx
        .client
        .get_with_query("/v1/directory/roles", &Value::Object(query))
        .await?;
