...
/// `pc organization department list` 的参数。
#[derive(Debug, Args)]
pub struct ListArgs {
...
/// 分页获取部门列表：`GET /v1/directory/departments`
/// （scope: `pcp:read:global:team`）。
///
/// 响应为分页结构（`page_index` / `page_size` / `total` / `values`）。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getDirectoryDepartments
pub async fn run(ctx: &Ctx, args: &ListArgs) -> anyhow::Result<()> {
    ...
    let response: Value = ctx
        .client
        .get_with_query("/v1/directory/departments", &Value::Object(query))
        .await?;
