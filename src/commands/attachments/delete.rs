...
pub struct DeleteArgs {
    ...
    /// Comment id; pass when deleting an attachment that belongs to a comment
    #[arg(long, value_name = "ID")]
    pub comment_id: Option<String>,
}

/// 删除一个附件：`DELETE /v1/attachments/{attachment_id}`（scope 依赖附件所属主体）。
///
/// 查询参数 `principal_type`（附件主体类型）、`principal_id`（主体 id）必填，
/// 删除评论附件时还需传 `comment_id`。返回被删除的附件对象。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/deleteAttachmentsByAttachmentIdByPrincipalTypeAndPrincipalId
