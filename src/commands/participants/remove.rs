...
pub struct RemoveArgs {
    ...
    /// Type of the principal to remove the participant from
...
/// 移除一个关注人：`DELETE /v1/participants/{participant_id}`
/// （scope 依赖关注人所属主体）。
///
/// 查询参数 `principal_type`（主体类型）、`principal_id`（主体 id）。
/// 返回被移除的关注人对象。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/deleteParticipantsByParticipantIdByPrincipalTypeAndPrincipalId
