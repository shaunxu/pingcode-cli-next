...
/// 分页获取产品中的需求优先级：`GET /v1/ship/idea/idea_priorities`
/// （查询参数 `product_id`，scope: `pcp:read:ship:idea`）。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getShipIdeaIdeaPrioritiesByProductId
pub async fn run(ctx: &Ctx, args: &ListForProductArgs) -> anyhow::Result<()> {
    ...
    let response: Value = ctx
        .client
        .get_with_query("/v1/ship/idea/idea_priorities", &Value::Object(query))
        .await?;
