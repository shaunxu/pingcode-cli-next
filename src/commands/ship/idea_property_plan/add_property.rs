...
/// `pc ship idea-property-plan add-property` 的参数。
#[derive(Debug, Args)]
...
/// 向需求属性方案中添加一个属性：`POST /v1/ship/idea_property_plans/{property_plan_id}/idea_properties`
/// （scope: `pcp:write:ship:configuration`）。
///
/// 请求体字段见官方文档（通常为属性引用及方案内配置）。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/postShipIdeaPropertyPlansByPropertyPlanIdIdeaProperties
