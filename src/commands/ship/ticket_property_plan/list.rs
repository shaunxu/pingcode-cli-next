...
/// `pc ship ticket-property-plan list` 的参数。
#[derive(Debug, Args)]
pub struct ListArgs {
...
/// 分页获取全部工单属性方案：`GET /v1/ship/ticket_property_plans`
/// （scope: `pcp:read:ship:configuration`）。
///
/// 文档：https://developer.alpha.pingcode.live/restapi/pingcode/getShipTicketPropertyPlans
pub async fn run(ctx: &Ctx, args: &ListArgs) -> anyhow::Result<()> {
    ...
    let response: Value = ctx
        .client
        .get_with_query("/v1/ship/ticket_property_plans", &Value::Object(query))
        .await?;
