...
3. **CI 自动发版**：merge 到 main 触发 `.github/workflows/release-tag.yml`（**手写**文件，可自由修改），分 `live-test` 与 `tag` 两个 job。门禁顺序：(a) 仅 release 提交（`github.event.head_commit.message` 含 `chore(release): v`，squash/rebase/merge commit 三种合并方式均覆盖）才跑 live-test job——**普通 feat/fix PR 合并不跑测试也不打 tag**；(b) `live-test` job 跑 `./scripts/live-test.sh` 全量 live 旅程（凭据同手动运行：`PC_CLIENT_ID`/`PC_CLIENT_SECRET` secrets、`PC_OPEN_API_BASE_URL` variable），**失败则 tag job 不执行，后续什么都不发生**（不产生空 GitHub Release）；(c) `tag` job 从 `Cargo.toml` 读版本，tag `vX.Y.Z` 已存在则直接成功（幂等），脚本内再校验 head 提交消息含 `chore(release): <tag>`。通过全部门禁后创建 annotated tag 并通过 `RELEASE_PAT` secret 推送。**必须用 PAT**：Actions 默认 `GITHUB_TOKEN` 推送的 tag 不会触发 `on: push: tags` 的其他 workflow（GitHub 防事件递归规则）；且 checkout 持久化的 `http.extraHeader`（GITHUB_TOKEN）优先级高于 push URL 内嵌凭据，脚本必须先 `git config --unset-all http.https://github.com/.extraheader`，否则会以 `github-actions[bot]` 身份推送被 403 拒绝。`RELEASE_PAT` 是 fine-grained token（仅授权本仓库、Contents: Read and write），配在仓库 Settings → Secrets and variables → Actions。tag 推送后 cargo-dist 的 release workflow 接管构建分发；失败可在 Actions 页面 re-run（幂等），或用 Actions 页面的 **Run workflow**（`workflow_dispatch`）手动补发（默认仍跑 live-test；勾选 `skip-tests` 可跳过测试直接补发，仍校验 tag 是否已存在）。
   - **为什么 live 门禁在打 tag 之前而不用 cargo-dist 的 `plan-jobs`**：dist 0.32.0 生成的 `release.yml` 中，`plan-jobs` 只接入 `build-local-artifacts` 的 needs，而 `host` job 的 `if` 用 `always()` 且把「因上游失败而 skipped」与「条件不满足正常 skipped」一视同仁——gate 失败时构建被跳过、`host` 却照常执行，会在没有任何产物的情况下创建空 GitHub Release（v1.2.0 事故即此）。tag 是 `release.yml` 的唯一触发源，在 `release-tag.yml` 打 tag 前卡住即可从源头杜绝，且不需要手改生成文件。
...
- 发版工具链分四层：
  ...
  - `.github/workflows/release-tag.yml`：release PR 合并后先跑 live-test 门禁，通过后创建 annotated tag `vX.Y.Z` 并用 `RELEASE_PAT` 推送（见上方第 3 步）。
